...
merge_data <- merge(idhm, populacao, by="Lugar")

idhm91 <- as.data.frame(cbind(merge_data$Lugar,merge_data$COD.IBGE.y,
...
idhm00 <- as.data.frame(cbind(merge_data$Lugar,merge_data$COD.IBGE.y,
...
idhm10 <- as.data.frame(cbind(merge_data$Lugar,merge_data$COD.IBGE.y,
                              merge_data$IDHM..2010.,merge_data$IDHM.Renda..2010.,merge_data$IDHM.Longevidade..2010.,
                              merge_data$IDHM.Educação..2010.,merge_data$POP2010))

colnames(idhm10) <- c("municipio","cod","idhm","idhm_renda","idhm_longev","idhm_edu","pop")
